...
上面这种情况，只有`master`一个主分支就显得不够用了。通常，你不得不在`master`分支以外，另外新建一个`production`分支跟踪线上版本；或者结合简化Git Flow工作流 / 丰富功能分支工作流，提前做好约定；或者利用Github的**标签 + release**功能定期发布线上版本源代码与程序，而`master`用来存储和展示最新代码。

但是，最主流的的方案还是保证master分支上的代码是随时可部署或发布的，tag和release只是用来记录过去发布版本的；这其实是要求管理者面对所有功能分支（commit或PR）合并到主分支的操作时，都必须经过严格的测试和审查才可合并进来。
