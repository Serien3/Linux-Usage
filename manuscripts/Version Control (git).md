...
Git 将顶级目录中的文件和文件夹作为集合，并通过一系列快照来管理其历史记录。在 Git 的术语里，被追踪的文件被称作 Blob 对象（数据对象），也就是一组数据。目录则被称之为“树”，它将名字与 Blob 对象或树对象进行映射（使得目录中可以包含其他目录）。快照则是被追踪的最顶层的树，它可以被看作是带时间戳的文件夹副本。例如，一个树看起来可能是这样的：
...
上面是一个 ASCII 码构成的简图，其中的 `o` 表示一次提交（快照）。（继承关系）
...
文件，文件夹 -> Blob对象，树 -> 快照（顶层树）

从时间角度看，历史记录是一个由快照组成的有向无环图快照是一个commit

Blob，树（空间），commit（时间）均是对象，对象需要名字索引（哈希）

对象加上它的名字引用就是Git仓库

以上均是**不可变**的！**不可变性**是Git版本控制的核心！
...
在我们使用Git进行工作时，每次输入的命令不止会对历史记有影响。要注意它对我们的工作目录（也就是当前磁盘上的代码，包括已经被Git仓库跟踪的文件和未被跟踪的本地文件），暂存区和历史记录（尤其是Git的文件系统和历史记录，它们共同构成了Git数据模型）都会造成什么变化，这是记忆和掌握Git的命令行接口的关键。
...
- `git checkout <revision>`: 更新 HEAD 和目前的分支（将HEAD指针转移，指向`<reversion>`提交），并让**整个git仓库中的文件和`<revision>`提交完全一致**。值得注意的是，这里说的是git仓库中的文件，工作区的未被追踪的文件或代码改动是叠加在快照之上的，只要不冲突就会一起保留
...
- `git branch`: 显示本地仓库的所有分支（本质上就是引用）
  ...
  - 注意：分支大致可以分为本地分支和远程跟踪分支。远程跟踪分支是远程仓库中的分支的引用，它们缓存在本地仓库中，可以与本地分支关联绑定（也可以不绑定，只是缓存在本地提示远程仓库的情况）。这些远程分支在本地是只读的引用，不能直接在上面提交更改。
...
2. `git merge--squash branch`
   ...
   Squash压缩合并：把 `branch` 的所有提交合并到当前分支，但只产生一个新的合并提交，当前分支的提交历史中不会保留 `branch` 的多次提交记录，即不会记录它和当前分支的合并关系。特别注意，当我们使用 --squash 选项执行 merge 时，Git 不会像在正常合并中那样直接在目标分支中创建一个合并提交，而是把合并后的结果放在当前工作目录和缓冲区中（默认 `--no-commit`），需要自己手动 `git commit` 一次。
...
3. `git rebase branch`
   ...
   **在实际开发中，可以认为合并主要在以下两个场景使用**

   - 在私有（本地的、未推送的）分支上使用 `rebase`来做合并（将本地功能分支推送到远程共享分支之前，使用 `rebase` 将其基于最新的主分支进行更新），以保持提交历史整洁。
   - 在共享（已推送到远程仓库并供多人协作）的分支上使用 `merge`来做合并（将一个干净、线性的功能分支合并到主分支时）以避免重写历史记录带来的协作风险。并使用`--no-ff`来生成一个显式的合并提交，以明确标记功能集成的点，保持历史可读性。
...
# 如果你使用git rebase发生了冲突，那你这里只能用git rebase -continue
...
- `git clone <url> <folder name>`: 从远端下载仓库并创建自己的本地副本(`<folder name>`默认为当前文件夹)
  ...
  - `-b <分支名>` 或 `--branch <分支名>`：指定要克隆后**立即检出**的特定分支活tag，而不是远程的默认分支。
...
- `git add -p`: 交互式暂存文件的片段
  ...
  -  暂存区存在的意义在于**精确控制提交粒度**，不无脑的`git add .`，而是每次提交有选择地选取某些文件甚至某些语句，这样会让历史记录的粒度更好，与实际工作分离开。
...
- `git stash`: 用于将工作区和暂存区的未提交修改（必须是来自已跟踪文件的，若要增加未跟踪文件，需添加`-a`参数）临时保存到一个独立的「stash 栈」中，同时将工作区恢复到当前提交（HEAD）的干净状态。
...
上面这种情况，只有`master`一个主分支就显得不够用了。通常，你不得不在`master`分支以外，另外新建一个`production`分支跟踪线上版本；更主流的做法是，利用Github的**标签 + release**功能定期发布线上版本源代码与程序，或者使用包管理器发布版本，而`master`用来存储和展示最新开发成果。

但是，以上一切的核心在于，必须保证master分支上的代码是随时可部署或发布的（release-ready），tag和release只是用来表达某个历史稳定发布版本的；这其实是要求管理者面对所有功能分支（commit或PR）合并到主分支的操作时，都必须经过严格的测试和审查才可执行。
