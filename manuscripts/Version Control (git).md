...
在我们使用Git进行工作时，每次输入的命令不止会对历史记录（Git数据模型）有影响。要注意它对我们的**工作目录（修改区，也就是当前磁盘上的代码）**，**暂存区**和**历史记录（数据模型）**都会造成什么变化，这是记忆和掌握Git的命令行接口的关键。
...
- `git checkout <revision>`: 更新 HEAD 和目前的分支（将HEAD指针转移，指向`<reversion>`提交），并让**整个工作目录中的文件和`<revision>`提交完全一致**
...
2. `git merge--squash branch`
   ...
   有趣的事情是，当你当前工作区在A分支上，并修改了代码，但忘记暂存和提交，这时将工作区切换到B分支上，会相当于在B分支上自动执行了一次`git merge--squash branchA`。
